...
setprop persist.sys.collectlog.state 1
...
[ -d  /data/vendor/ssrdump ] && cp -a /data/vendor/ssrdump $APLOG_DIR/
...
#catch bugreports by default by yangjianbin
#if [ $(getprop persist.sys.bugreport_enable) = true ]; then
if true; then
    setprop persist.sys.catch.bugreport true
    while :
    do
        sleep 5
        if [ $(getprop persist.sys.bugreport.state) = 1 ]; then
        sleep 5
        Log "Collecting bugreport"
        else
        Log "collect bugreport done"
        break
        fi
    done
fi
...
for svc in mainlog radiolog kernellog eventslog modemlog tcpDumplog adsplog; do
...
for svc in mainlog radiolog kernellog eventslog tcpDumplog adsplog; do
    if eval [ "\$$svc" = "running" ]; then
        start $svc
    fi
done
...
setprop persist.sys.collectlog.state 0
