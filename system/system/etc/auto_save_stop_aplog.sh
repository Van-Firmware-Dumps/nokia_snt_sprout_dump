...
do
  ...
  if [ -n "$(getprop init.svc.tcpDumplog)" ]; then
    if [ $(getprop init.svc.tcpDumplog) = "running" ]; then
      echo "tcpDumplog is running." >> $LOGFILE
      TCPDUMP_LOG_STATUS=running
    else
      echo "tcpDumplog is stopped" >> $LOGFILE
      TCPDUMP_LOG_STATUS=stopped
    fi
  else
    echo "tcpDumplog not run" >> $LOGFILE
    TCPDUMP_LOG_STATUS=stopped
  fi
  ...
  if [ "$TCPDUMP_LOG_STATUS" = "stopped" ]; then
    echo "tcpDumplog services stopped, return shell right now." >> $LOGFILE
    return
  fi

  for svc in radiolog eventslog;
  do
...
stop tcpDumplog
echo "tcpDumplog stopped by timeout" >> $LOGFILE
echo "" >> $LOGFILE
wait
